...
#elif defined(HAVE_GCRYPT)
  gcry_md_close (self->priv->checksum);
#else
...
#elif defined(HAVE_GCRYPT)
static int
gchecksum_type_to_gcrypt (GChecksumType checksum_type)
{
  switch (checksum_type)
    {
    case G_CHECKSUM_SHA256:
...
#elif defined(HAVE_GCRYPT)
  stream->priv->checksum_type = gchecksum_type_to_gcrypt (checksum_type);
  g_assert (!gcry_md_open (&stream->priv->checksum, stream->priv->checksum_type, 0));
#else
...
#elif defined(HAVE_GCRYPT)
      gcry_md_write (self->priv->checksum, buffer, res);
#else
...
#elif defined(HAVE_GCRYPT)
  guchar *digest = gcry_md_read (stream->priv->checksum, 0);
  g_assert (digest != NULL);
  memcpy (buffer, digest, *digest_len);
  if (digest_len)
    *digest_len = gcry_md_get_algo_dlen (stream->priv->checksum_type);
  gcry_md_reset (stream->priv->checksum);
#else
...
#elif defined(HAVE_GCRYPT)
  gsize len = gcry_md_get_algo_dlen (stream->priv->checksum_type);
  guchar *ret = g_malloc0 (len);
  guchar *src = gcry_md_read (stream->priv->checksum, 0);
  g_assert (src != NULL);
  memcpy (ret, src, len);
  gcry_md_reset (stream->priv->checksum);
#else
...
ot_checksum_instream_get_string (OtChecksumInstream *stream)
{
  ...
  return buf;
#elif (defined(HAVE_GNUTLS) || defined(HAVE_GCRYPT))
  gsize len;
